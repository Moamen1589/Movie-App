...
export default function Popular() {
    ...
    useEffect(() => {
        fetch(`https://api.themoviedb.org/3/movie/popular?language=en-US&page=${num}`, options)
            .then(res => res.json())
    ...
    return (
        <>
            <h1 className="text-white text-center text-5xl my-8">Popular</h1>
            <Swiper
                slidesPerView={4}
                navigation
                breakpoints={{
                ...
                modules={[Navigation]}
                loop={true}
                className="parent  flex justify-center mx-auto"
            >
                {films.results.map((film) => (
                    <SwiperSlide key={film.id} >
                        <div
                            className=" poster  w-65 relative   object-contain  hover:scale-110 duration-500 rounded-2xl cursor-pointer"
                        ...
                        >
                            {/* " poster  w-65 relative   object-contain  hover:scale-110 duration-500 rounded-2xl cursor-pointer" */}
